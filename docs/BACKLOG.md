...
- **~8 stale "Gemini #2" human-name strings in core comments/logs** — *Unclassified — cosmetic* — P3.5-4 swept module symbols (`clip_assigner`/`gemini2_assign`) but the grep token set missed the human-name string "Gemini #2". Live-surfaced in the smoke log (e.g. `pipeline.py:404/411` "using full-pool Gemini #2" — code runs the packer; the string lies). Spots: `pipeline.py` ×2 (log msgs), `assets/retrieval.py:253`, `assign/__init__.py:1`, `beat_planner.py:3`, `clip_assignment_validator.py` comments, `clip_retriever.py:5,80`, `gemini_client.py:85`, `steps.py` docstrings. Behavior-correct, doc-only.
- **Word floor 150→145 consider** — *Tentative — arsenal* — smoke saw 3/6 videos draw <150 (145/147/149) and retry; all recovered, finals 152-160. Lowering the LONG floor to 145 would cut the retry friction on thin-material POIs. Leo's standing deferred call.
- **Speed: bump render concurrency** — *Tentative* — smoke ran `--concurrency 2` on an 8-core VPS (6 cores idle, load ~3) → render-bound, sequential, ~56 min for 6 videos. `PROMO_RENDER_CONCURRENCY=6` should roughly halve render time. Note `--tail-workers` only helps when upscale is ON (overlaps the long tail); with `--final-upscale-provider disabled` it buys nothing. Long-term render lever is the ffmpeg-vs-Remotion swap (ROADMAP §6).
