...
class TestSprint10C2RegenerateSingleVariant:
    """C2: regenerate_single_variant_with_hint pipes the hint through
    ...
    def test_hint_reaches_prompt_and_regen_returns_validated_script(
        ...
        original_build = script_generator._build_prompt
        ...
        with patch.object(script_generator, "_build_prompt", side_effect=capturing_build), \
             patch.object(script_generator, "resolve_gemini_model", return_value=object()), \
             patch.object(script_generator, "_generate_one",
                          return_value=self._valid_long_raw_script()):
            result = script_generator.regenerate_single_variant_with_hint(
                poi_name="Test Hotel",
                location="Nowhere",
                clips_metadata=clips_metadata,
                persona_path=persona_path,
                ...
                n_variants=2,
                variant_plan=None,
                tighten_hint="Segment 3 required 4.25s; tighten segment 3",
                n_candidates=1,
                max_retries=1,
                target_duration_sec=65,
            )
        # Hint made it into the prompt.
        assert seen_prompts, "capturing_build was never invoked"
        assert "FEEDBACK FROM PIPELINE" in seen_prompts[0]
        assert "tighten segment 3" in seen_prompts[0].lower()
        # Regen returned a normalized, validated script.
        assert result["variant_index"] == 1
        assert result["target_duration_sec"] == 65
        assert result["poi_name"] == "Test Hotel"
        assert "total_words" in result

    def test_regen_raises_when_all_attempts_fail_validation(
        self, tmp_path, monkeypatch,
    ):
        from promo.core.script import script_generator
        ...
        with patch.object(script_generator, "resolve_gemini_model", return_value=object()), \
             patch.object(script_generator, "_generate_one", return_value=None):
            with pytest.raises(RuntimeError, match="F3 regen exhausted"):
                script_generator.regenerate_single_variant_with_hint(
                    poi_name="Test Hotel",
                    location="Nowhere",
                    clips_metadata=clips_metadata,
                    persona_path=persona_path,
                    ...
                    n_variants=2,
                    variant_plan=None,
                    tighten_hint="Segment 2 required 3.0s",
                    n_candidates=1,
                    max_retries=0,
                    target_duration_sec=65,
                )
