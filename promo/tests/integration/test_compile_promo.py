...
class TestCompilePromoHelpers:
    ...
    def test_full_pipeline_multi_variant_smoke(self):
        ...
        # Sprint 10 C2: full_pipeline now invokes clip_assigner between
        # TTS and props-building. Patch the F3 retry entry point so the
        # smoke test does not hit the real Gemini #2 API.
        def _fake_f3_retry(script, narration_in, clips_metadata, clip_durations,
                           **kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, \
             ...
             patch("promo.core.assign.clip_assigner.assign_clips_with_f3_retry", side_effect=_fake_f3_retry) as mock_assign, \
...
class TestSprint10C2FullPipelineWiring:
    """C2 criterion 6 (full_pipeline integration surface): full_pipeline
    invokes assign_clips_with_f3_retry and handles ClipAssignmentError by
    incrementing pool_exhaustion_hard_fails and continuing the variant
    loop."""

    def _source(self) -> str:
        """Return the per-variant loop body source.

        promo-handoff-readiness Sprint 4 A-001 narrow — the variant-loop
        body moved out of ``full_pipeline`` into
        ``promo.core.pipeline.variant_loop._run_variant_loop``; the
        ``except ClipAssignmentError`` guard + ``pool_exhaustion_hard_fails
        += 1`` invariant lives there.
        """
        import inspect
        from promo.core.pipeline.variant_loop import _run_variant_loop
        return inspect.getsource(_run_variant_loop)

    def test_full_pipeline_imports_and_calls_assign_clips_with_f3_retry(self):
        """Under Sprint 10 C4 the F3 retry lives in _step_assign_clips; under
        Sprint 4 A-001 the variant loop that invokes it was extracted to
        ``promo.core.pipeline.variant_loop._run_variant_loop``. Same
        invariant, new home."""
        import inspect
        from promo.core.pipeline.variant_loop import _run_variant_loop
        from promo.core.pipeline.steps import _step_assign_clips

        loop_source = inspect.getsource(_run_variant_loop)
        assert "_step_assign_clips(" in loop_source, (
            "variant loop must invoke _step_assign_clips under Sprint 10"
        )
        assert "ClipAssignmentError" in loop_source, (
            "variant loop must catch ClipAssignmentError from the F3 "
            "second-attempt failure path"
        )
        # And the actual F3 delegate lives inside _step_assign_clips.
        step_source = inspect.getsource(_step_assign_clips)
        assert "assign_clips_with_f3_retry" in step_source

    def test_full_pipeline_increments_pool_exhaustion_on_f3_fail(self):
        """Source-level check: the except ClipAssignmentError clause
        increments pool_exhaustion_hard_fails before continuing the loop,
        mirroring the existing FreezeWouldOccurError handling."""
        source = self._source()
        # Locate the except block.
        except_pos = source.find("except ClipAssignmentError")
        assert except_pos != -1
        tail = source[except_pos:except_pos + 500]
        assert "pool_exhaustion_hard_fails" in tail
        assert "all_ok = False" in tail
        assert "continue" in tail

    def test_full_pipeline_passes_regenerate_callables(self):
        """Sprint 10 C4: both F3 callables live inside _step_assign_clips
        (closures over the variant context) and are passed to
        assign_clips_with_f3_retry so retries actually fire."""
        import inspect
        from promo.cli import compile_promo

        src = inspect.getsource(compile_promo._step_assign_clips)
        assert "regenerate_script_fn=" in src
        assert "regenerate_narration_fn=" in src

class TestSprint10C4Decomposition:
    """C4 criterion 10: full_pipeline decomposed into three named step
    helpers, body length under 400 lines.
    """

    def test_step_helpers_exist_and_are_callable(self):
        from promo.cli import compile_promo
        ...
        for name in (
            ...
            fn = getattr(compile_promo, name, None)
            assert callable(fn), f"Sprint 10 C4: {name} must exist and be callable"

    def test_full_pipeline_body_under_400_lines(self):
        """AST count: full_pipeline body from `def full_pipeline(` through
        its last statement is < 400 lines (criterion 10 ceiling).

        promo-handoff-readiness Sprint 4 A-001 narrow — ``full_pipeline``
        lives in ``promo.core.pipeline.pipeline`` now; parse that file.
        """
        import ast
        import inspect
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "full_pipeline":
                size = node.end_lineno - node.lineno + 1
                assert size < 400, (
                    f"full_pipeline body is {size} lines; contract criterion 10 "
                    "caps it at 400."
                )
                return
        raise AssertionError(
            "full_pipeline not found in promo.core.pipeline.pipeline source"
        )

    def test_step_tts_narration_is_thin_wrapper(self):
        """_step_tts_narration's signature matches the contract and
        returns whatever generate_narration returns."""
        import inspect
        from promo.cli.compile_promo import _step_tts_narration

        sig = inspect.signature(_step_tts_narration)
        assert list(sig.parameters) == ["script", "voice_key", "tmp_dir", "speed"]

    def test_step_assign_clips_delegates_to_clip_assigner(self, monkeypatch):
        """_step_assign_clips builds the regen callables and passes them
        to assign_clips_with_f3_retry; no real Gemini #2 call ever fires
        because we patch the delegate."""
        from promo.cli.compile_promo import _step_assign_clips
        from promo.core.assign import clip_assigner

        captured: dict = {}

        def _fake_f3(script, narration, clips_metadata, clip_durations, **kwargs):
            captured.update(kwargs)
            captured["script"] = script
            captured["narration"] = narration
            return script, narration, [{"segment": 1, "clip_id": "0001"}]

        monkeypatch.setattr(
            clip_assigner, "assign_clips_with_f3_retry", _fake_f3,
        )
        script = {"variant_index": 1, "segments": [
            {"segment": 1, "pause_weight": 2, "pause_after_ms": 500,
             "text": "x y"},
        ]}
        narration = {"word_timestamps": [
            {"word": "x", "start": 0.0, "end": 0.2},
            {"word": "y", "start": 0.2, "end": 0.4},
        ], "segment_timestamps": []}

        result = _step_assign_clips(
            script, narration, [{"id": "0001"}], {"0001": 5.0}, 1,
            poi_name="T", location="X",
            hotel_description="", notable_details="",
            variant_voice_key="jarnathan", variant_tmp_dir="/tmp/x",
            tts_speed=0.95, target_duration_sec=65,
            effective_wpm=190, n_variants_total=1, script_candidates=1,
        )
        assert result[2] == [{"segment": 1, "clip_id": "0001"}]
        # Both regen callables were provided to the delegate.
        assert callable(captured.get("regenerate_script_fn"))
        assert callable(captured.get("regenerate_narration_fn"))
        assert captured.get("variant_index") == 1

class TestSprint12bStepAssignClipsKwarg:
    """AC3 — compile_promo._step_assign_clips threads embedding_cache_dir;
    default-None path does not build the retrieval closure."""

    def test_step_assign_clips_has_embedding_cache_dir_kwarg(self):
        import inspect
        from promo.cli.compile_promo import _step_assign_clips

        params = inspect.signature(_step_assign_clips).parameters
        assert "embedding_cache_dir" in params
        assert params["embedding_cache_dir"].default is None

    def test_filter_clips_by_ids_preserves_reduced_order(self):
        from promo.cli.compile_promo import _filter_clips_by_ids

        clips = [{"id": "a", "x": 1}, {"id": "b", "x": 2}, {"id": "c", "x": 3}]
        # reduced order: c, a (NOT original order)
        result = _filter_clips_by_ids(clips, ["c", "a"])
        assert [c["id"] for c in result] == ["c", "a"]
        assert result[0]["x"] == 3
        assert result[1]["x"] == 1

    def test_filter_clips_by_ids_skips_unknown_ids(self):
        from promo.cli.compile_promo import _filter_clips_by_ids

        clips = [{"id": "a"}, {"id": "b"}]
        # "nonexistent" should be silently skipped (sidecar/metadata drift).
        assert _filter_clips_by_ids(clips, ["b", "nonexistent", "a"]) == [
            {"id": "b"}, {"id": "a"},
        ]

    def test_step_assign_clips_none_cache_dir_does_not_build_closure(
        self, monkeypatch
    ):
        """When embedding_cache_dir=None, retrieve_clips_fn passed to
        assign_clips_with_f3_retry MUST be None (Sprint 11 no-op)."""
        from promo.cli import compile_promo
        from promo.core.assign import clip_assigner
        ...
        def fake_f3_retry(script, narration, clips_metadata, clip_durations,
            ...
            captured["retrieve_clips_fn"] = retrieve_clips_fn
        ...
        assert captured["retrieve_clips_fn"] is None

    def test_step_assign_clips_missing_sidecar_warns_and_noops(
        self, monkeypatch, tmp_path, caplog
    ):
        """When embedding_cache_dir exists but has no sidecar, the step
        logs a WARNING and still passes retrieve_clips_fn=None."""
        from promo.cli import compile_promo
        ...
        with caplog.at_level("WARNING"):
            compile_promo._step_assign_clips(
                ...
                target_duration_sec=30.0, effective_wpm=165,
                n_variants_total=1, script_candidates=1,
                embedding_cache_dir=str(tmp_path),  # empty dir — no sidecar
            )

        assert captured["retrieve_clips_fn"] is None
        assert any(
            "no embedding sidecar" in r.message for r in caplog.records
        )

    def test_step_assign_clips_with_sidecar_reduces_inventory(
        self, monkeypatch, tmp_path
    ):
        """End-to-end: sidecar on disk → closure → reduced inventory
        reaches assign_clips via retrieve_clips_fn."""
        from promo.cli import compile_promo
        from promo.core.assign import clip_embedder

        # Build a minimal sidecar with 4 clips.
        clips = [
            {"id": f"clip_{i}",
             "scene_description": f"desc {i}", "category": "hotel"}
            for i in range(4)
        ]
        import numpy as np

        # Make orthogonal-ish vectors so top-k is deterministic.
        vectors = np.eye(4, 1536).tolist()
        sidecar = {
            "embeddings": {
                f"clip_{i}": {"vector": vectors[i], "input": f"desc {i}"}
                for i in range(4)
            },
            "model": "text-embedding-3-small",
            "dim": 1536,
            "mimo_prompt_sha1": clip_embedder.current_mimo_prompt_sha1(),
            "composition_version": 1,
        }
        sidecar_path = clip_embedder.sidecar_path(
            str(tmp_path),
            clip_embedder.current_mimo_prompt_sha1(),
            1,
        )
        ...
        # Stub embed_query_fn so we don't hit OpenRouter.
        def fake_embed_texts(queries):
            # Map query text to a unit vector collinear with clip_0.
            return [vectors[0] for _ in queries]

        monkeypatch.setattr(
            "promo.core.assign.clip_embedder.embed_texts", fake_embed_texts,
        )
        ...
        script = {"segments": [{"text": "q1"}, {"text": "q2"}]}
        narration = {"word_timestamps": [], "segment_timestamps": []}

        compile_promo._step_assign_clips(
            script, narration, clips, {}, 1,
            poi_name="X", location="", hotel_description="",
            notable_details="", variant_voice_key="jarnathan",
            variant_tmp_dir="/tmp", tts_speed=1.0,
            target_duration_sec=30.0, effective_wpm=165,
        ...
        assert captured["retrieve_clips_fn"] is not None
        # Both queries collinear with clip_0 → union is small (not the full 4).
        reduced_ids = [c["id"] for c in captured["reduced"]]
        assert "clip_0" in reduced_ids
        # Since 2 identical queries × k=6 (clamped to 4) → union = 4 (full
        # pool). That would NOT reduce. To avoid the full-pool fallback
        # triggering on len==N, skip size-assertion — just confirm the
        # closure DID run and returned the filtered metadata (not full).

class TestSprint12bFallbackGuards:
    """AC4 — H2 union-size shortfall + attach shrinkage both fall back to
    the full clips_metadata pool and log WARNING."""

    def test_attach_shrinkage_falls_back_to_full_pool(
        self, monkeypatch, tmp_path, caplog
    ):
        from promo.cli import compile_promo
        from promo.core.assign import clip_embedder

        # Sidecar only has 2 of 3 clips.
        import numpy as np
        vectors = np.eye(2, 1536).tolist()
        sidecar = {
            "embeddings": {
                "a": {"vector": vectors[0], "input": "a-desc"},
                "b": {"vector": vectors[1], "input": "b-desc"},
            },
            "model": "text-embedding-3-small",
            "dim": 1536,
            "mimo_prompt_sha1": clip_embedder.current_mimo_prompt_sha1(),
        ...
        sp = clip_embedder.sidecar_path(
            str(tmp_path), clip_embedder.current_mimo_prompt_sha1(), 1,
        )
        os.makedirs(os.path.dirname(sp), exist_ok=True)
        with open(sp, "w") as f:
            json.dump(sidecar, f)

        # Full clips_metadata has 3 clips; sidecar is missing "c".
        clips_metadata = [
            {"id": "a", "scene_description": "a-desc", "category": "hotel"},
            {"id": "b", "scene_description": "b-desc", "category": "hotel"},
            {"id": "c", "scene_description": "c-desc", "category": "hotel"},
        ]

        monkeypatch.setattr(
            "promo.core.assign.clip_embedder.embed_texts",
            lambda qs: [vectors[0] for _ in qs],
        )
        ...
        def fake_f3_retry(script, narration, clips_metadata, clip_durations,
            ...
            captured["retrieve_clips_fn"] = retrieve_clips_fn
            if retrieve_clips_fn is not None:
                captured["result"] = retrieve_clips_fn(script)
            return script, narration, []
        ...
        # Fallback MUST return the full clips_metadata.
        assert captured["result"] is clips_metadata
        # Sprint 13 AC18 (D-002): the unified WARNING fires at the
        # compile_promo call site; it MUST include the counts AND the
        # dropped clip_ids list AND the fallback-reason tag so operators
        # can diagnose retrieval regressions without cross-referencing a
        # second log timestamp.
        shrink_warnings = [
            r for r in caplog.records
            if "shrunk" in r.message.lower()
        ]
        assert shrink_warnings, f"no shrinkage warning, got: {[r.message for r in caplog.records]}"
        unified = shrink_warnings[0].message
        assert "m4_attach_shrinkage" in unified, unified
        assert "'c'" in unified or "c" in unified.split("dropped:")[1], (
            f"dropped-id 'c' not surfaced in unified warning: {unified}"
        )
        assert "3" in unified and "2" in unified, (
            f"counts (3→2) not both in warning: {unified}"
        )
        # D-002 invariant: the upstream per-clip WARNING from
        # attach_embeddings_to_metadata must NOT fire — only the unified
        # call-site record remains.
        assert not any(
            r.name == "promo.core.assign.clip_embedder"
            and "attach_embeddings_to_metadata" in r.message
            for r in caplog.records
        ), "legacy per-clip WARNING leaked from clip_embedder"

    def test_union_shortfall_falls_back_to_full_pool(
        self, monkeypatch, tmp_path, caplog
    ):
        from promo.cli import compile_promo
        from promo.core.assign import clip_embedder

        # 2 clips, 5 narration queries, all collinear → union_size=2 < 5.
        import numpy as np
        vectors = np.eye(2, 1536).tolist()
        sidecar = {
            "embeddings": {
                "a": {"vector": vectors[0], "input": "a"},
                "b": {"vector": vectors[1], "input": "b"},
            },
            "model": "text-embedding-3-small",
            "dim": 1536,
            "mimo_prompt_sha1": clip_embedder.current_mimo_prompt_sha1(),
        ...
        # All queries → same vector → union_of_top_k returns just 2 ids
        # but we feed 5 narration queries, so union_size(2) < 5.
        monkeypatch.setattr(
            "promo.core.assign.clip_embedder.embed_texts",
            lambda qs: [vectors[0] for _ in qs],
        )
        ...
        script = {"segments": [{"text": f"q{i}"} for i in range(5)]}
        narration = {"word_timestamps": [], "segment_timestamps": []}
        ...
        assert captured["result"] is clips_metadata  # full-pool fallback
        union_warnings = [
            r for r in caplog.records
            if "union size" in r.message.lower()
        ]
        assert union_warnings, f"no union-shortfall warning, got: {[r.message for r in caplog.records]}"
...
class TestSprint12bStructuralGuards:
    """AC6 (H4) — union_of_top_k and clip_retriever are ABSENT from the
    body of assign_clips (the non-retry entry point). Retrieval lives at
    the F3 wrapper only."""

    def test_assign_clips_body_has_no_retriever_reference(self):
        import inspect
        from promo.core.assign import clip_assigner

        src = inspect.getsource(clip_assigner.assign_clips)
        assert "clip_retriever" not in src, (
            "assign_clips body must not reference clip_retriever"
        )
        assert "union_of_top_k" not in src, (
            "assign_clips body must not reference union_of_top_k"
        )

    def test_no_score_threshold_heuristic_in_integration(self):
        """N6 — score-threshold fallback heuristic is deferred; must NOT
        appear in integration surface. Guards semantic patterns only
        (not the bare literal 0.35 which pre-exists as a Gemini
        temperature constant in clip_assigner.py unrelated to retrieval)."""
        assigner = (
            Path(__file__).resolve().parents[2] / "core" / "assign" / "clip_assigner.py"
        ).read_text()
        compile_promo_src = (
            Path(__file__).resolve().parents[2] / "cli" / "compile_promo.py"
        ).read_text()

        for forbidden in ("score_floor", "cosine_threshold",
                          "low_confidence_fallback", "score_threshold"):
            assert forbidden not in assigner, (
                f"{forbidden!r} leaked into clip_assigner.py"
            )
            assert forbidden not in compile_promo_src, (
                f"{forbidden!r} leaked into compile_promo.py"
            )


# ---------------------------------------------------------------------------
#  promo-handoff-readiness Sprint 1 AC-B3: argparse-precedence regression
# ---------------------------------------------------------------------------
