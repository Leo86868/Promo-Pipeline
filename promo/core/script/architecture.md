...
- **`HOOK_TECHNIQUES`** — 6 hook seed strings (`contradiction`, `sensory`, `specific_number`, `second_person`, `time_anchor`, `superlative`) rotated across variants to diversify the script openers. Currently hard-coded in `script_prompt_builder.py`; tracked in [`BACKLOG.md`](../../../BACKLOG.md) for arsenal externalization.
...
- **`HOOK_TECHNIQUES` is Python-resident** — 6 hook seed strings hard-coded in `script_prompt_builder.py`; tracked in [`BACKLOG.md`](../../../BACKLOG.md) for arsenal externalization.
