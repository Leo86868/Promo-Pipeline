...
> **Read upstream first:** [`README.md`](../../../README.md) → [`promo/core/architecture.md`](../architecture.md) (defines POI, sidecar, Gemini #1, why assignment runs after TTS, facade re-export pattern, pause_weight). This doc covers stage 2 only.
...
- **Facade re-export pattern** — `script_generator.py` is the single import path tests + callers target. Sibling symbols re-exported up so `monkeypatch.setattr(script_generator, "_generate_one", ...)` resolves through the facade's globals. Same constraint as `narrate/tts_engine`.
