...
| `openrouter_text_client.py` | **Provides:** `resolve_openrouter_text_model(log_context=...)` (returns an `OpenRouterTextModel` handle; fail-loud on missing `OPENROUTER_API_KEY`), `generate_text(model, prompt, generation_config=...)` (chat/completions call, returns text ≡ SDK `response.text`), `map_generation_config(...)` (GenAI→OpenAI param map). **In:** prompt + GenAI-style `generation_config`. **Out:** completion text. **Side:** no `requests`/SDK import of its own — issues HTTP via `model_adapters.openrouter.post_chat_completion` (the `requests` boundary), mirroring how `clip_embedder` reaches OpenRouter. Reads `OPENROUTER_SCRIPT_MODEL` via `os.getenv` (same carve-out shape as `GEMINI_MODEL`). **Raises:** `RuntimeError` on empty completion; `ValueError` on an unmapped `generation_config` key. **Consumers:** `model_adapters/gemini` (billing-failover branch of the switch, selected by `config.script_llm_provider()`). |
| `retry.py` | **Provides:** `retry_with_backoff(func, max_retries=3, base_delay=1.0, max_delay=60.0, exceptions=(Exception,))` — wraps any callable and retries on the listed exceptions with exponential backoff (delay doubles per attempt, capped at `max_delay`). **In:** the callable + retry policy parameters. **Out:** the callable's return value. **Side:** sleeps between attempts; logs each attempt. **Raises:** the last caught exception after `max_retries` failures. **Consumers (4 sites):** `analyze/clip_analyzer` (MiMo via OpenRouter), `script/script_gemini_caller` (Gemini #1), `assign/clip_embedder` (OpenRouter embeddings), `narrate/tts_elevenlabs` (ElevenLabs HTTP). |
| `json_response.py` | **Provides:** `parse_json_response(text)` — strict dict-shape JSON parser for AI text responses. **In:** raw response text. **Out:** parsed `dict`. **Side:** strips ` ```json ... ``` ` fences before parsing; pure otherwise. **Raises:** `ValueError` on JSON decode failure or non-dict top-level shape. **Consumers:** `script/script_gemini_caller` only — the sole stage whose LLM reply is parsed as a JSON object. |

## How they wire together

**Cross-file seams:**

- `GEMINI_API_KEY` routes through `config.gemini_api_key()` (typed resolver, fail-fast). `GEMINI_MODEL` is the documented carve-out from Pluggability Charter Rule 2: read directly via `os.getenv("GEMINI_MODEL", registry.GEMINI_TEXT_MODEL)` inside `gemini_client` because that module sits one import-cycle hop away from `config`.
- `model_adapters/gemini` exports `GeminiModel` / `resolve_gemini_model`, owns the SDK call wrapper used by the script stage (Gemini #1), and is the **provider switch point**: `config.script_llm_provider()` (`PROMO_SCRIPT_LLM_PROVIDER`, default `gemini`) decides whether `resolve_gemini_model` returns a Google `GeminiModel` or an `OpenRouterTextModel` (the `OPENROUTER_API_KEY` billing-failover lane for the same `gemini-2.5-pro`); `generate_content_text` dispatches on the handle type. Script code stays provider-agnostic and, with the env unset, the SDK path is byte-identical.

**Invariants:**

- **Single quarantine site** — no other module is allowed to `import google.generativeai`. New consumers route through `model_adapters/gemini`. The failover provider is a **sibling** quarantine module (`openrouter_text_client.py`) that imports neither the SDK nor `requests` (it borrows `model_adapters.openrouter`'s HTTP boundary) — proving the charter's "add a sibling module" extension path.
...
- **`parse_json_response` forbids top-level lists by design** — catches Gemini drifts that would silently parse but break downstream typed consumers.
