...
- **clip assignment (stage 4)** — deterministic code (beat planner → retrieval → packer → validator), not an LLM. Picks which clip plays during which beat AFTER the script has been spoken, so timing is measured, not predicted. Lives in `assign/`.
- **Why assignment runs after TTS?** — picking clips before TTS would mean predicting word timing at prompt time. Running assignment on the real measured `word_timestamps` removes the prediction.
...
- **facade re-export pattern** — one Python file (`tts_engine.py`, `script_generator.py`) acts as the public import surface; it re-imports private helpers from sibling files so external test code can patch them by short name. (`clip_assigner.py` retired with the legacy LLM-assigner chain (1f28902) on 2026-06-11; the assign stage no longer needs a facade.)
- **two-space model** — the project's load-bearing invariant. *Assigner space*: the script's last word ends at `narration_end`; the assignment only worries about covering the script up to `narration_end`. *Renderer space*: the final video has to be at least `target_duration_sec` long. The gap is filled by extra clips called *bridges*. See [/architecture.md](../../architecture.md) "Two-space model" for the formal treatment.
...
- **F3 retry (RETIRED 2026-06-11)** — was the "single retry" policy for the legacy LLM-assigner's clip-assignment failures (script rewrite + re-TTS + one more attempt). The deterministic assigner has no equivalent failure mode; `ClipAssignmentError` now aborts the variant directly (resume re-renders).
- **per-beat retrieval** — each beat's text is embedded once and ranked against the POI's clip embeddings; the packer consumes that ranking as candidate order. The ranking is advisory: the validator gates on coverage, not on `clip_id ∈ retrieved_ids`.
...
|---|---|
| **Gemini #1** | The `gemini-2.5-pro` call that generates the narration script. Clip assignment is deterministic code, not a Gemini call. |
...
| `assign/` | 4. Assign (deterministic, 翻转二) | **In:** `Script + Narration + clips_metadata + clip_durations` (+ clip→asset mapping). **Out:** `(script, narration, list[ClipAssignment], provenance)`. **Side:** one batched embedding call; usage-window ledger read (fail-closed). **Raises:** `ClipAssignmentError` / `UsageWindowError` / `RuntimeError` (all variant-abort). |
...
| `errors.py` | **Provides:** 5 named exception types — each carries a recovery contract: `ClipAssignmentError` (raised by `assign/` packer/validator, **no retry → variant abort**; `--resume` re-renders); `FreezeWouldOccurError` (raised by `render/`, **no retry → variant abort**); `ForcedAlignmentError` (raised by `narrate/forced_aligner` on Gemini path, **no retry → variant abort**); `MimoAnalysisError` (raised by `analyze/` after retry budget exhausted, **aborts the run**); `NoSuitableBGMError` (raised by `pipeline/bgm_voice_resolver`, **aborts before any variant runs**). **Consumers:** every stage raises one or more; `pipeline/` and `cli/` decide abort vs recover. |
...
The five stages are sequential within one variant; `pipeline/full_pipeline` runs the run-level pre-loop (clip prep + voice/BGM + Gemini #1 + pause budget) and `pipeline/_run_variant_loop` runs the per-variant inner loop (TTS + deterministic assignment + render). `selection/`, `llm/`, and `arsenal_loader.py` cut across stages.
...
| `script/` | `system_prompts/gemini1_script_v1.md` (Gemini #1 prompt), `script_skeletons/*.yaml` (format templates), `personas/*.yaml` (narrator personas) | Gemini SDK + retry helper + JSON-response parser (all three) — `script/` is the only stage that uses the full `llm/` surface. |
...
| `assign/` | nothing — the deterministic assign stage reads no arsenal prompt | retry helper alone, for `clip_embedder`'s OpenRouter embedding calls. No Gemini SDK — assignment makes no LLM call. |
...
- **`assign/` (`pipeline/steps._assign_clips_packer`)** — In: `Script`, `Narration`, `clips_metadata`, `clip_durations`, `shared_assets` mapping, supplied by `pipeline/_step_assign_clips`. Out: `(script, narration, list[ClipAssignment], provenance)` — script/narration pass through untouched. Side: one batched embedding call (inline/sidecar/platform ladder); usage-window ledger read (fail-closed, 设计契约 ②). Raises: `ClipAssignmentError` / `UsageWindowError` / `RuntimeError` — all abort the variant; `--resume` re-renders.
...
- `pipeline/_run_variant_loop` owns **per-variant** ordering — TTS (Step 4) → deterministic assignment (Step 4.5) → props build + freeze prevention (Step 7) → Remotion render (Step 8) → success-gated observability row append.
...
- **Errors** — 5 named exceptions, all in `errors.py`. Recovery taxonomy: `ClipAssignmentError` → variant abort (resume re-renders); `FreezeWouldOccurError` / `ForcedAlignmentError` → variant abort; `MimoAnalysisError` → run abort (after retry exhaustion); `NoSuitableBGMError` → run abort (pre-variant). Every doc surface above carries a "Raises" field — if any disagrees with this taxonomy, one of the docs is wrong.
