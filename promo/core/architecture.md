...
- **facade re-export pattern** — one Python file (`tts_engine.py`, `script_generator.py`) acts as the public import surface; it re-imports private helpers from sibling files so external test code can patch them by short name. (`clip_assigner.py` retired with the Gemini #2 chain on 2026-06-11; the assign stage no longer needs a facade.)
...
- **F3 retry (RETIRED 2026-06-11)** — was the "single retry" policy for Gemini #2 clip-assignment failures (script rewrite + re-TTS + one more attempt). The deterministic assigner has no equivalent failure mode; `ClipAssignmentError` now aborts the variant directly (resume re-renders).
